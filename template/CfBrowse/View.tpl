...
<table cellspacing=0 border=0 width='100%'>
	...
	<tr><td>
		<input type="text" name="pageLimit" style="width:50px;" value="{:$pageLimit:}" onChange='form.submit();'
			{:if !is_null($rowKeys) :}disabled{:/if:}
			>
	</td></tr>
	<tr><th align='left'>
		BrowseMode
	...
	<tr>
		<td>
			...
			<input type="hidden" name="rowKeys[]"value="{:$rowKey:}"/>
			{:/foreach:}
			{:/if:}
				...
				<tr class='bg_list{:$index % 2 + 1:}'
					...
					<td width='100%'>
						...
						<table class='rest_table'  cellpadding=2 width='100%'>
							{:foreach $kvsList[$key] as $column => $value :}
							<tr>
								<th class='rest_menu' width='50%'>{:$column:}</th>
								<td class='rest_{:$_ % 2 +1 :}' width='50%' nowrap>{:$value:}</td>
							</tr>
							{:/foreach:}
						</table>
						{:else:}
